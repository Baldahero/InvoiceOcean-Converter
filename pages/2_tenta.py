...
for row_index, transaction in enumerate(transactions, start=1):
    matched_buyer = find_best_client_match(transaction.counterparty_name, clients)
    buyer = matched_buyer or transaction.counterparty_name
...
for output_index, (_, row) in enumerate(edited_frame.iterrows(), start=1):
    ...
    client_key = buyer if buyer in clients else find_best_client_match(buyer, clients)
